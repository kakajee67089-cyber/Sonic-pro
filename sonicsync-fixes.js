(function(){
  ...
  // Phone login is the only auth method removed. Email, Google and Guest stay intact.
  ...
  function mobileCSS(){
    ...
    s.textContent=`
      *,*::before,*::after{box-sizing:border-box}
      ...
      #ss-room-radar{position:fixed;left:50%;bottom:78px;transform:translateX(-50%);z-index:2147482990;width:min(430px,calc(100vw - 20px));max-height:55vh;overflow:auto;padding:12px;border-radius:18px;background:rgba(12,16,32,.97);border:1px solid rgba(129,140,248,.35);box-shadow:0 20px 70px rgba(0,0,0,.45);color:#fff;display:none}
      #ss-room-radar.show{display:block}
      #ss-room-radar .ssrr-user{display:flex;align-items:center;gap:8px;padding:8px 0;border-bottom:1px solid rgba(255,255,255,.08)}
      #ss-room-radar .ssrr-avatar{width:34px;height:34px;border-radius:50%;overflow:hidden;display:grid;place-items:center;background:linear-gradient(135deg,#6366f1,#ec4899);font-weight:900;flex:none}
      #ss-room-radar .ssrr-avatar img{width:100%;height:100%;object-fit:cover}
      @media(max-width:520px){
        .tb{padding-left:8px!important;padding-right:8px!important}
        .tbl{max-width:42vw!important;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
        .bnav{height:68px!important}
        .nb .ni{font-size:22px!important}
        .nb .nl{font-size:9px!important}
        #pg-game{padding-bottom:82px!important}
        #pg-game .card{padding:12px!important}
      }
    `;
  ...
  function installModePicker(){
    ...
    ov.innerHTML=`<div class="ssmp-card" role="dialog" aria-modal="true" aria-labelledby="ssmp-title">
      <div style="font-size:28px">🎮</div><div id="ssmp-title" style="font-size:20px;font-weight:900;margin-top:4px">How do you want to play?</div>
      <div class="ssmp-grid"><button id="ssmp-solo">🧑 Solo</button><button id="ssmp-multi">👥 Multiplayer</button></div>
      <button id="ssmp-cancel" class="secondary" style="width:100%;margin-top:9px">Cancel</button>
    </div>`;
    document.body.appendChild(ov);
    let action={solo:null,multi:null};
    ...
    $('ssmp-solo').onclick=()=>{const f=action.solo;close();safe(()=>f&&f())};
    $('ssmp-multi').onclick=()=>{const f=action.multi;close();safe(()=>f&&f())};
    $('ssmp-cancel').onclick=close;
    ov.addEventListener('click',e=>{if(e.target===ov)close()});
    window.__ssOpenMode=open;
  }
  ...
  function captureGameModeClicks(){
    document.addEventListener('click',function(e){
      ...
      const page=b.closest('.page');if(!page)return;
      if(page.id==='pg-memoryrush' && (txt.includes('solo')||txt.includes('create room')||txt.includes('with a friend')||txt.includes('multiplayer'))){
        e.preventDefault();e.stopImmediatePropagation();window.__ssOpenMode('Memory Rush',window.startMemoryRushSolo,window.createMemoryRushRoom);return;
      }
      if(page.id==='pg-mindsnap' && (txt.includes('solo')||txt.includes('create duel')||txt.includes('friend')||txt.includes('multiplayer'))){
        e.preventDefault();e.stopImmediatePropagation();window.__ssOpenMode('Mind Snap Duel',window.startMindSolo,window.createMindRoom);return;
      }
      if(page.id==='pg-cardflip' && (txt.includes('solo')||txt.includes('create room')||txt.includes('friend')||txt.includes('multiplayer'))){
        e.preventDefault();e.stopImmediatePropagation();window.__ssOpenMode('Card Flip',()=>window.startFCGame?.('solo'),window.createFCRoom);return;
      }
    },true);
  }
  ...
  function installSocketRoomEvents(){
    const old=window.ssInitSocket;if(typeof old!=='function'||old.__ssV12)return;window.ssInitSocket=function(){const r=old.apply(this,arguments);setTimeout(()=>{const s=window.SS_SOCKET;if(!s||s.__ssRoomV12)return;s.__ssRoomV12=true;s.on('game:request-start',p=>{if(p?.roomId!==window.mrRoomId&&p?.roomId!==window.msRoomId)return;const ok=confirm((p.name||'Player')+' wants to start the game. Accept?');if(ok)s.emit('game:start',{roomId:p.roomId,game:p.game});});s.on('room:kicked',p=>{if(!p?.roomId)return;window.toast?.('You were removed from the room.');if(p.roomId===window.mrRoomId){window.mrRoomId=null;window.mrSolo=false}if(p.roomId===window.msRoomId){window.msRoomId=null;window.msSolo=false}window.goPage?.('game')});s.on('game:started',p=>{if(!p?.roomId)return;if(p.roomId===window.mrRoomId&&window.mrApplyServerRound){}if(p.roomId===window.msRoomId)window.msStarted=true});s.on('connect',()=>safe(()=>s.emit('presence:register',{uid:window.U?.uid,name:window.U?.name,photo:window.U?.photo||null,email:window.U?.email||null})))},120);return r};window.ssInitSocket.__ssV12=true}
  }

  function fixFindOpponent(){
    const old=window.findOpponent;if(typeof old!=='function'||old.__ssV12)return;
    window.findOpponent=function(){if(window.mmSearching)return;window.mmSearching=true;['mm-find-btn'].forEach(id=>$(id)&&( $(id).style.display='none'));if($('mm-cancel-btn'))$('mm-cancel-btn').style.display='block';if($('mm-status'))$('mm-status').style.display='block';if($('mm-status-text'))$('mm-status-text').textContent='Searching for '+window.mmType+' opponent…';const s=window.SS_SOCKET;if(s&&s.connected){window.__ssMMHandlers();s.emit('matchmaking:join',{game:window.mmType,uid:window.U?.uid,name:window.U?.name,photo:window.U?.photo||null})}else old.apply(this,arguments)};window.findOpponent.__ssV12=true;
    window.__ssMMHandlers=function(){const s=window.SS_SOCKET;if(!s||s.__ssMMV12)return;s.__ssMMV12=true;s.on('matchmaking:queued',p=>{if($('mm-status-text'))$('mm-status-text').textContent='Waiting for another '+(p.game||window.mmType)+' player…'});s.on('matchmaking:matched',p=>{window.mmSearching=false;clearInterval(window.mmTimer);if($('mm-status-text'))$('mm-status-text').textContent='🎉 Opponent found: '+(p.opponent?.name||'Player');window.toast?.('🎉 Opponent found!');if(p.game==='math'||p.game==='color'){window.quizRoomId=p.roomId;window.quizIsHost=p.hostUid===window.U?.uid;window.quizMode='multi';window.quizPendingType=p.game==='color'?'color':'math';window.quizPendingOp='mix';if(window.showQuizWaiting)window.showQuizWaiting(p.hostUid===window.U?.uid,p.roomId);if(p.hostUid===window.U?.uid&&window.createHostedQuizRoom){}else if(window.joinQuizRoom){const i=$('qjoinp');if(i)i.value=p.roomId;window.joinQuizRoom()}}});};
  }

  function patchSocialNotifications(){
    if(window.__ssSocialV12)return;window.__ssSocialV12=true;let last='';setInterval(()=>safe(async()=>{if(!window.U?.uid||typeof window.dbGet!=='function')return;const me=await window.dbGet('users/'+window.U.uid);if(!me)return;const notes=Object.entries(me.notifications||{}).reverse();const fresh=notes.find(([k,n])=>n&&n.read===false&&Number(n.time||0)>Date.now()-10000&&k!==last);if(fresh){const [key,n]=fresh;last=key;window.toast?.(n.type==='like'?'❤️ Someone liked your profile':n.type==='follow'?'👥 Someone followed you':n.type==='request'?'🤝 Friend request received':'🔔 New activity');window.dbUpdate?.('users/'+window.U.uid+'/notifications/'+key',{...n,read:true})}}),3000);
  }

  function boot(){
    removePhoneAuth();mobileCSS();installModePicker();captureGameModeClicks();patchClocks();patchNotifications();installPullRefresh();patchSocketStatus();patchRoomSearch();installPublicRoomSearch();patchRoomMemberUX();installSocketRoomEvents();fixFindOpponent();patchSocialNotifications();
    setTimeout(()=>{removePhoneAuth();patchClocks();patchNotifications();patchRoomSearch();installPublicRoomSearch();patchRoomMemberUX()},800);
    setInterval(()=>safe(()=>{patchClocks();patchNotifications();patchSocketStatus();installPublicRoomSearch();patchRoomMemberUX()}),1500);
  }
