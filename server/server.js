...
// SonicSync Pro realtime backend v12.
...
app.get('/',(_q,res)=>res.json({ok:true,service:'SonicSync Socket.IO',auth:'firebase',project:PROJECT_ID}));
app.get('/health',(_q,res)=>res.json({ok:true,service:'SonicSync Socket.IO',auth:'firebase',project:PROJECT_ID,now:Date.now()}));
...
function ensureRoom(id,type='music',hostUid=null){if(!rooms.has(id))rooms.set(id,{roomId:id,type,hostUid,users:new Map(),sockets:new Map(),playback:null,gameState:null,gameData:{},createdAt:Date.now()});const r=rooms.get(id);if(type)r.type=type;if(hostUid&&!r.hostUid)r.hostUid=hostUid;return r;}
...
function emitState(r,s=null){const st={roomId:r.roomId,type:r.type,hostUid:r.hostUid,users:publicUsers(r),playback:r.playback,gameState:r.gameState};(s||io.to(r.roomId)).emit('room:state',st);}
...
function createMatch(game,a,b,aSocketId,bSocketId){const roomId=(Math.random().toString(36).slice(2,8)+Date.now().toString(36).slice(-2)).toUpperCase().replace(/[^A-Z0-9]/g,'').slice(0,6);const r=ensureRoom(roomId,game,a.uid);r.hostUid=a.uid;r.users.set(a.uid,a);r.users.set(b.uid,b);r.gameState={started:false};for(const [sid,u] of [[aSocketId,a.uid],[bSocketId,b.uid]]){if(!sid)continue;const so=io.sockets.sockets.get(sid);if(so){r.sockets.set(sid,u);so.join(roomId);so.data.rooms=so.data.rooms||new Set();so.data.rooms.add(roomId);}}emitState(r);emitUsers(r);return roomId;}
function queueMatch(s,payload){const game=String(payload?.game||'math');const uid=s.data.uid;if(!uid)return s.emit('matchmaking:error',{message:'Authentication required'});let q=matchQueue.get(game);if(!q){q=new Map();matchQueue.set(game,q)}for(const [otherUid,e] of q){if(otherUid===uid)continue;const me=s.data.user||{uid,name:String(payload.name||'Player'),photo:payload.photo||null,email:s.data.email||''};const opp=e.user;const roomId=createMatch(game,opp,me,e.socketId,s.id);q.delete(otherUid);io.to(s.id).emit('matchmaking:matched',{roomId,game,hostUid:otherUid,opponent:opp});io.to(e.socketId).emit('matchmaking:matched',{roomId,game,hostUid:otherUid,opponent:me});return;}q.set(uid,{uid,user:{uid,name:String(payload.name||s.data.user?.name||'Player'),photo:payload.photo||s.data.user?.photo||null,email:s.data.email||''},socketId:s.id,time:Date.now()});s.emit('matchmaking:queued',{game});}
...
function leaveRoom(socket,idRaw,uidRaw){const id=String(idRaw||'').toUpperCase(),r=rooms.get(id);if(!r)return;const uid=r.sockets.get(socket.id)||String(uidRaw||'');r.sockets.delete(socket.id);try{socket.leave(id)}catch(e){}if(uid&&!([...r.sockets.values()].includes(uid)))r.users.delete(uid);if(r.hostUid===uid)r.hostUid=[...r.users.keys()][0]||null;emitUsers(r);emitState(r);removeEmpty(id);}
server.listen(PORT,()=>console.log(`SonicSync Socket.IO Firebase-auth v12 running on port ${PORT}`));
